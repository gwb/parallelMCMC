parallel.tempering.fast <- function(targets.ls, single.rproposals.ls, single.dproposals.ls, init.states.mat, niter){
    ...
    for(i in seq(nchains)){
        X.ls[[i]] <- matrix(0, nrow=niter, ncol=ncol(init.states.mat))
        L.ls[[i]] <- matrix(0, nrow=niter, ncol=ncol(init.states.mat))
        R.ls[[i]] <- vector('numeric', niter)
    }
