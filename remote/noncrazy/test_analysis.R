...
compute.weight.i <- function(i){
    ...
    x2 <- rq2(5000)
    res.i <- bridge.sampling.very.fast(constrained.targets[[i]], dq2, res.ls[[i]], x2)
    return(list(res.i))
}
...
run.simul <- function(i){
    ...
    wis <- unlist(lapply(seq(3), function(i) compute.weight.i(i)))
    wis <- wis / sum(wis)
...
res <- do.call('rbind', mclapply(seq(2), run.simul, mc.cores=16))
